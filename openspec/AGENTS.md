...
3. 使用 `## ADDED|MODIFIED|REMOVED|RENAMED Requirements` 起草规范增量，每个需求至少包含一个 `#### Scenario:`。
...
- MODIFIED：改变现有需求的行为、范围或验收标准。始终粘贴完整的、更新后的需求内容（标题 + 所有场景）。归档器将用您在这里提供的内容替换整个需求；部分增量将丢失以前的细节。
- RENAMED：仅在名称改变时使用。如果您也改变行为，使用 RENAMED（名称）加上 MODIFIED（内容）引用新名称。

常见错误：使用 MODIFIED 添加新关注点而不包含以前的文本。这会在归档时导致细节丢失。如果您没有明确改变现有需求，而是在 ADDED 下添加新需求。
